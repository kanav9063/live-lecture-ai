...
### 🌟 New in v2.0.1: The Intelligence & Stability Update

We've significantly upgraded Natively's core engine and premium experience (incorporating all v2.0.0 and v2.0.1 enhancements):

#### 1. Next-Generation Profile Engine (Premium)

_How it helps:_ The new Profile Engine features a stunning, Apple-like minimalist UI with fluid glassmorphism. It effortlessly analyzes your resume against job descriptions to dynamically prompt the AI.
_The Natively Advantage:_ Competitors like Cluely or Final Round AI rely on generic LLM prompts, resulting in robotic, impersonal interview answers. Natively's Profile Engine integrates your _exact_ experience timeline, ensuring the AI responses sound exactly like you.

#### 2. Live Meeting RAG (Retrieval-Augmented Generation)

_How it helps:_ During active calls, Natively now instantly retrieves and correlates information from both the current ongoing meeting and your entire past meeting history, answering complex queries flawlessly.
_The Natively Advantage:_ While other meeting assistants only look at the last few sentences, Natively cross-references facts instantly across thousands of past meeting transcripts stored entirely locally.

#### 3. Bulletproof Windows Audio Architecture

_How it helps:_ Windows users often struggle with audio routing. We've completely re-engineered our native Rust audio layer to support robust WASAPI capture and granular device selection.
_The Natively Advantage:_ Unlike web-based wrappers or simpler tools that drop audio during intensive background processing, Natively guarantees zero-drop, system-level audio capture directly from Zoom, Teams, or Meet—even on standard laptops.

#### 4. Apple Premium Dark Code Experience

_How it helps:_ Reading code during a high-stakes technical interview requires absolute clarity. We've introduced a gorgeous Apple Premium Dark aesthetic for all markdown and code blocks, flawlessly preserving structure and whitespace.

#### 5. Rock-Solid Foundation & PR Merges

_How it helps:_ Merged 3 major community PRs and fixed critical bugs in SQLite embeddings, native module rendering, and Markdown formatting, forming a highly reliable application packaging infrastructure.
